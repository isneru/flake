 ...
 * Author: omerwk (https://github.com/omerwk/samaritan-sddm-theme)
 *
 * Vendored copy, three changes from upstream:
 * - the background image gets a QtQuick.Effects MultiEffect blur plus a
 *   dark dim overlay (see the "Background image" block below) - upstream
 *   has neither, since the theme is designed around a plain black
 *   background by default
 * - the ~6s fictional boot-text sequence (hardcoded "CORE INITIALIZATION"/
 *   "GOVERNMENT FEEDS ACTIVE" strings, not real logs) is removed entirely -
 *   interfaceReady now defaults true, so the real title/panels/samaritan/
 *   login staggered reveal starts immediately instead of after it
 * - the "GOVERNMENT FEEDS" status-panel label (paired with the real kernel
 *   version value) is relabeled "KERNEL" - it was real data under a
 *   nonsensical fictional label, unlike the theme's other deliberately-
 *   fictional stats (THREAT ANALYSIS, THREATS IDENTIFIED, MODE), which are
 *   untouched
 * utils/SystemInfo.js is also fully regenerated (see sddm.nix) with this
 * host's real hostname/CPU/kernel/OS instead of upstream's own machine.
 * Everything else is upstream's Main.qml verbatim.
 */
import QtQuick 2.0
...
Rectangle {
    ...
    // interfaceReady defaults true (was false, flipped true by a ~6s boot
    // sequence) - that sequence was pure decorative fiction (hardcoded
    // "CORE INITIALIZATION"/"GOVERNMENT FEEDS ACTIVE" text, not real system
    // logs) gating every real element behind a multi-second wait for no
    // functional reason. Removed entirely; the staggered title/panels/
    // samaritan/login reveal below (its own real feature) now starts
    // immediately instead.
    property bool titleVisible: false
    property bool interfaceReady: true
    ...
    // Background image - blurred, with a dark dim overlay for text contrast.
    // Upstream has neither: this theme is designed around a plain black
    // background by default (theme.conf: "leave [backgroundImage] empty to
    // use background color"), so a real photo wallpaper needs some
    // treatment to keep the terminal-style text readable over it.
    Image {
    ...
    Rectangle {
        anchors.fill: parent
        color: "black"
        opacity: mainRect.backgroundImage !== "" ? 0.5 : 0
    }
