{ style, pkgs, ... }:
let
  customPlugins = builtins.fetchGit {
    url = "https://github.com/isneru/vesktop-custom-plugins";
    rev = "40ff629c0c7c3c228dd2a1b967282af9de64bda2";
  };

  customVencord = pkgs.vencord.overrideAttrs (old: {
    preBuild = (old.preBuild or "") + ''
      mkdir -p src/userplugins
      for dir in ${customPlugins}/*/; do
...

{
  xdg.configFile."vesktop/themes/theme.css".text = ''
    :root, .theme-dark {
      --font-primary: "${style.fonts.mono}", ${
        builtins.concatStringsSep ", " (map (f: ''"${f}"'') style.fonts.fallbacks)
      };
  ...
  programs.vesktop = {
    ...
    settings = {
      ...
      firstLaunch = false;
      arRPC = "on";
      splashColor = "rgb(${style.colors.hexToRgbValues style.colors.accent})";
      splashBackground = "rgb(${style.colors.hexToRgbValues style.colors.bgDim})";
      splashTheming = true;
      minimizeToTray = "on";
      closeToTray = "on";
